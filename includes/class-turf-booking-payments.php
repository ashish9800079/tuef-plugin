...
class Turf_Booking_Payments {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_process_razorpay_payment', array($this, 'process_razorpay_payment'));
        add_action('wp_ajax_nopriv_process_razorpay_payment', array($this, 'process_razorpay_payment'));
        
        add_action('wp_ajax_verify_razorpay_payment', array($this, 'verify_razorpay_payment'));
        add_action('wp_ajax_nopriv_verify_razorpay_payment', array($this, 'verify_razorpay_payment'));
    }
    ...
    public function process_razorpay_payment() {
        ...
        $amount = $booking_details['payment_amount'] * 100; // Razorpay expects amount in paise
        ...
        if (isset($body['error'])) {
            wp_send_json_error(array('message' => $body['error']['description']));
        }
    ...
    public function verify_razorpay_payment() {
        ...
        $response = wp_remote_get('https://api.razorpay.com/v1/payments/' . $razorpay_payment_id, $args);
        ...
        if (isset($payment_data['error'])) {
            wp_send_json_error(array('message' => $payment_data['error']['description']));
        }
            ...
            <div class="tb-payment-details">
                ...
                <table class="tb-payment-details-table">
                    ...
                    <tr>
                        ...
                        <td><?php echo esc_html($booking_details['date']); ?></td>
                    </tr>
                    <tr>
                        <th><?php _e('Time', 'turf-booking'); ?></th>
                        <td><?php echo esc_html($booking_details['time_from'] . ' - ' . $booking_details['time_to']); ?></td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            ...
                            $general_settings = get_option('tb_general_settings');
                            $currency_symbol = isset($general_settings['currency_symbol']) ? $general_settings['currency_symbol'] : '₹';
                            echo esc_html($currency_symbol . number_format($booking_details['payment_amount'], 2));
                            ?>
                        </td>
                    </tr>
                </table>
            </div>
            <button id="razorpay-checkout-button" class="tb-button"><?php _e('Pay Now', 'turf-booking'); ?></button>
        </div>
